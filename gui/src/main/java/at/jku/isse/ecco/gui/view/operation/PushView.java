...
public class PushView extends OperationView {
	...
	public PushView(EccoService service) {
		super();
		...
		this.pushButton.setOnAction(event -> {
			...
			Task pushTask = new Task<Void>() {
				...
				public void succeeded() {
					super.succeeded();
					PushView.this.stepSuccess("Push operation was successful.");
					PushView.this.setDisable(false);
				}

				@Override
				public void cancelled() {
					super.cancelled();
					PushView.this.stepError("Push operation was cancelled.", this.getException());
					PushView.this.setDisable(false);
				}
				...
				public void failed() {
					super.failed();
					PushView.this.stepError("Error during push operation.", this.getException());
					PushView.this.setDisable(false);
				}
			};
			PushView.this.progressBar.progressProperty().bind(pushTask.progressProperty());
			this.stepProgress();
	...
	private void stepRemote() {
		...
		this.rightButtons.getChildren().setAll(selectButton, pushButton);
	...
	private void stepDeselect() {
		...
		this.rightButtons.getChildren().setAll(pushButton);
