...
public class RuntimeModuleTest {
    ...
    //private static final String diretorio = "C:\\Users\\gabil\\Desktop\\Bonus_Work\\Classes";
    //private static final File diretorioScenario = new File("C:\\Users\\gabil\\eclipse-workspace\\ArgoUMLSPLBenchmark");
    ...
    public void Create_file_withLineNumbers_for_each_Class() {
        try {
            File runtimetraces = new File("C:\\Users\\gabil\\Desktop\\DynamicFeatureLocation\\Results\\test-tree-comparison\\traces\\collaboration08fev2020.trace");
            ArrayList<String> coveredClasses = new ArrayList<String>();
            FileReader fileReader = new FileReader(runtimetraces);
            BufferedReader br = new BufferedReader(fileReader);
            String linha;
            while ((linha = br.readLine()) != null) {
                if (!(linha.contains("### BTrace Log: ") || linha.equals("") || linha.equals(" "))) {
                    String className = linha.substring(0, linha.indexOf(" "));
                    //int aux = txt.lastIndexOf('.') + 1;
                    //String className = txt.substring(aux);
                    String test, test2;
                    if (className.contains("$")) { //because of nested classes
                        test2 = className.substring(0, className.indexOf("$")).replace(".", "\\");
                        //test = className.substring(className.lastIndexOf('.')+1, className.indexOf("$")) + ".runtime";
                        test = className.substring(0, className.indexOf("$")) + ".runtime";
                    } else {
                        test2 = className.replace(".", "\\");
                        //test = className.substring(className.lastIndexOf('.')+1) + ".runtime";
                        test = className + ".runtime";
                    }
                    String path = "C:\\Users\\gabil\\Desktop\\DynamicFeatureLocation\\Results\\test-tree-comparison\\COLLABORATIONDIAGRAM.config\\" + test;
                    ...
                    if (!file.exists()) {
                        file.createNewFile();
                        String text = linha.substring(linha.indexOf(" ") + 1);
                        ...
                        String linesAppended, stringToCompare = text.replace("\n", "");
                        boolean flag = true;
                        while ((linesAppended = br2.readLine()) != null) {
                            if (linesAppended.equals(stringToCompare)) {
                                flag = false;
                            }
                        }
                        br2.close();
                        if (flag) {
                            Files.write(Paths.get(path), text.getBytes(), new StandardOpenOption[]{StandardOpenOption.APPEND});
                        }
                        //appendStrToFile(file.getName(),text);
                    }
    ...
    private static final Path ORIGINAL_DIR = Paths.get("C:\\Users\\gabil\\eclipse-workspace\\ArgoUMLSPLBenchmark\\scenarios\\ScenarioOriginalVariant\\variants\\Original.config\\src");
    ...
    private static final Path SCENARIO_DIR = Paths.get("C:\\Users\\gabil\\Desktop\\DynamicFeatureLocation\\Results\\test-tree-comparison\\scenarioOriginal");
    // set this path to where the results should be stored
    private static final Path OUTPUT_DIR = Paths.get("C:\\Users\\gabil\\Desktop\\DynamicFeatureLocation\\Results\\test-tree-comparison\\scenarioOriginal");
    // set this path to where the results for a specific scenario should go
    private static final Path SCENARIO_OUTPUT_DIR = OUTPUT_DIR.resolve("results");
...
//				e.printStackTrace();
//			}
           // if (variantDir.getFileName().toString().contains("BASE")  ) {
