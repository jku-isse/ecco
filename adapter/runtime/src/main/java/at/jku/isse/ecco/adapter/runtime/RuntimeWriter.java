...
public class RuntimeWriter implements ArtifactWriter<Set<Node.Op>, Path> {
	...
	public Path[] write(Path base, Set<Node.Op> input) {
		...
		try {
			...
			while (i.hasNext()) {
				Node.Op tempNode = (Node.Op) i.next();
				...
				if (tempNode.getArtifact().getData() instanceof PluginArtifactData) {
					...
					List<Node.Op> children = (List<Node.Op>) tempNode.getChildren();
					try {
