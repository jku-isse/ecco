...
public class XmlModule implements Module, Serializable {
    ...
    public XmlModule(Feature[] pos, Feature[] neg) {
        requireNonNull(pos);
        requireNonNull(neg);
        assert (pos.length > 0);
        this.verify(pos, neg);
    ...
    public ModuleRevision addRevision(FeatureRevision[] pos, Feature[] neg) {
        if (!this.matchesRevision(pos, neg))
            return null;
        ModuleRevision moduleRevision = new XmlModuleRevision(this, pos, neg);
    ...
    public ModuleRevision getRevision(FeatureRevision[] pos, Feature[] neg) {
        return this.revisions.get(new XmlModuleRevision(this, pos, neg));
    }
    ...
    public boolean equals(Object o) {
        ...
        XmlModule xmlModule = (XmlModule) o;

        if (this.pos.length != xmlModule.pos.length || this.neg.length != xmlModule.neg.length)
            return false;
        for (int i = 0; i < this.pos.length; i++) {
            boolean found = false;
            for (int j = 0; j < xmlModule.pos.length; j++) {
                if (this.pos[i].equals(xmlModule.pos[j])) {
                    found = true;
                    break;
                }
            }
        ...
        for (int i = 0; i < this.neg.length; i++) {
            boolean found = false;
            for (int j = 0; j < xmlModule.neg.length; j++) {
                if (this.neg[i].equals(xmlModule.neg[j])) {
                    found = true;
                    break;
                }
            }
