 ...
 */
public interface SequenceGraphOld extends Persistable {
	 ...
	 */
	public interface Op extends SequenceGraphOld {
		 ...
		 */
		public default void sequence(Op other) {
		 ...
		 */
		//private
		default void assignNewSequenceNumbersRec(Node.Op sgn, boolean newPol) {
			if (sgn.getPol() == newPol) // already visited
				return;

			sgn.setPol(this.getPol()); // mark as visited

			for (Transition.Op child : sgn.getChildren()) {
		 ...
		 */
		default void alignSequenceGraph(Op other) {
		...
		default int alignSequenceGraphRecFast(SequenceGraph.Node.Op left, SequenceGraph.Node.Op right, int cost) {
			...
			// recursion case 1: left has no more nodes, right does
				...
				for (SequenceGraph.Transition.Op rightChildEntry : right.getChildren()) {
					int temp_cost = this.alignSequenceGraphRecFast(left, rightChildEntry.getValue(), cost + 1);
			...
			// recursion case 2: right has no more nodes, left does
				...
				for (SequenceGraph.Transition.Op leftChildEntry : left.getChildren()) {
					int temp_cost = this.alignSequenceGraphRecFast(leftChildEntry.getValue(), right, cost + 1);
				...
				for (SequenceGraph.Transition.Op leftChildEntry : left.getChildren()) {
					for (SequenceGraph.Transition.Op rightChildEntry : right.getChildren()) {
						...
						if (leftChildEntry.getKey().equals(rightChildEntry.getKey())) { // we found a match -> pursue it
							int temp_cost = this.alignSequenceGraphRecFast(leftChildEntry.getValue(), rightChildEntry.getValue(), cost); // do not increment cost as it was a match
...
//			}
				...
				// case 3: skip right
				for (SequenceGraph.Transition.Op rightChildEntry : right.getChildren()) {
					int temp_cost = this.alignSequenceGraphRecFast(left, rightChildEntry.getValue(), cost);
		 ...
		 */
		//private
		default int alignSequenceGraphRec(Node.Op left, Node.Op right, int cost) {
			int local_best_cost = Integer.MAX_VALUE;
			...
			// recursion case 1: left has no more nodes, right does
				...
				for (Transition.Op rightChildEntry : right.getChildren()) {
			...
			// recursion case 2: right has no more nodes, left does
				...
				for (Transition.Op leftChildEntry : left.getChildren()) {
		 ...
		 */
		//private
		default Node.Op updateSequenceGraphRec(Node.Op left, Node.Op right, HashSet<Artifact.Op<?>> path, Map<Set<Artifact.Op<?>>, Node.Op> nodes, Set<Artifact.Op<?>> shared_symbols) {
			// get current graph node
			Node.Op sgn = nodes.get(path);
			if (sgn == null) {
			...
			Map<Artifact.Op<?>, Node.Op> new_children = new HashMap<>();

			// if unshared symbol left -> advance
			for (Transition.Op leftEntry : left.getChildren()) {
				if (!shared_symbols.contains(leftEntry.getKey())) {
					...
					Node.Op child = this.updateSequenceGraphRec(leftEntry.getValue(), right, new_path, nodes, shared_symbols);
					new_children.put(leftEntry.getKey(), child);
				}
			}

			// if unshared symbol right -> add it left and advance
			for (Transition.Op rightEntry : right.getChildren()) {
				if (!shared_symbols.contains(rightEntry.getKey())) {
					System.out.println("RIGHT UNSHARED");
					HashSet<Artifact.Op<?>> new_path = new HashSet<>(path);
					new_path.add(rightEntry.getKey());
					Node.Op child = this.updateSequenceGraphRec(left, rightEntry.getValue(), new_path, nodes, shared_symbols); // this should be a new node
					new_children.put(rightEntry.getKey(), child);
				}
			}

			// if shared symbol -> cut it if onesided or take it when on both sides
			Iterator<Transition.Op> it = left.getChildren().iterator();
			while (it.hasNext()) {
				Transition.Op leftEntry = it.next();
				if (shared_symbols.contains(leftEntry.getKey())) {
					Transition.Op rightEntry = null;
					for (Transition.Op tempRightEntry : right.getChildren()) {
					...
					if (rightEntry != null) { // matching shared symbols -> take them
						...
						HashSet<Artifact.Op<?>> new_path = new HashSet<>(path);
						new_path.add(leftEntry.getKey());
						Node.Op child = this.updateSequenceGraphRec(leftEntry.getValue(), rightEntry.getValue(), new_path, nodes, shared_symbols);
						new_children.put(leftEntry.getKey(), child);
			...
			for (Map.Entry<Artifact.Op<?>, Node.Op> entry : new_children.entrySet()) {
		...
		//private
		default int alignSequenceRec(Node.Op left, List<? extends Artifact.Op<?>> artifacts, int node_right_index, int[] alignment, int cost) {
			...
			// first do the match
			if (node_right_index < artifacts.size() && left.getChildren().size() > 0) {
				for (Transition.Op entry : left.getChildren()) {
		...
		//private
		default Node.Op updateSequenceRec(HashSet<Artifact.Op<?>> path, Map<Set<Artifact.Op<?>>, Node.Op> nodes, Set<? extends Artifact.Op<?>> shared_symbols, Node.Op node, int alignment_index, List<? extends Artifact.Op<?>> aligned_nodes, boolean new_pol) {
			// get current graph node
			boolean new_node = false;
			Node.Op gn = nodes.get(path);
			if (gn == null) {
				//gn = new SequenceGraphNode(!new_pol);
				gn = (Node.Op) this.createSequenceGraphNode(!new_pol);
				nodes.put(path, gn);
				new_node = true;
			}
			...
			if (right != null && !shared_symbols.contains(right)) {
				...
				new_path.add(right);
				// take it
				Node.Op new_gn = updateSequenceRec(new_path, nodes, shared_symbols, node, alignment_index + 1, aligned_nodes, new_pol);
			...
			Iterator<Transition.Op> it = node.getChildren().iterator();
			while (it.hasNext()) {
				Transition.Op entry = it.next();
				...
				if (!shared_symbols.contains(entry.getKey())) {
					...
					// take it
					Node.Op new_gn = updateSequenceRec(new_path, nodes, shared_symbols, entry.getValue(), alignment_index, aligned_nodes, new_pol);
					...
					if (right != null && right.equals(entry.getKey())) {
						...
						new_path.add(entry.getKey());
						// take it
						Node.Op new_gn = updateSequenceRec(new_path, nodes, shared_symbols, entry.getValue(), alignment_index + 1, aligned_nodes, new_pol);
						// X not a new child. do nothing with new_children.
						//if (new_node)
						new_children.put(entry.getKey(), new_gn);
		...
		public default void updateArtifactReferences() {
			// update node list
			for (Node.Op sgn : this.collectNodes()) {
				// update references in children
				Map<Artifact.Op<?>, Node.Op> updatedChildren = new HashMap<>();
				Iterator<Transition.Op> childrenIterator = sgn.getChildren().iterator();
				while (childrenIterator.hasNext()) {
					Transition.Op childEntry = childrenIterator.next();
				...
				for (Map.Entry<Artifact.Op<?>, Node.Op> entry : updatedChildren.entrySet()) {
					sgn.addTransition(entry.getKey(), entry.getValue());
				}
			}
		}
		...
		//private
		default void updateArtifactReferencesRec(Node.Op sgn) {
			// update references in children
			Map<Artifact.Op<?>, Node.Op> updatedChildren = new HashMap<>();
			Iterator<Transition.Op> it = sgn.getChildren().iterator();
			while (it.hasNext()) {
				Transition.Op entry = it.next();
		...
		//private
		default void collectNodesRec(Node.Op sgn, Collection<Node.Op> nodes) {
			...
			for (Transition.Op entry : sgn.getChildren()) {
				this.collectNodesRec(entry.getValue(), nodes);
			}
		}


		public default Map<Set<Artifact.Op<?>>, Node.Op> collectPathMap() {
			Map<Set<Artifact.Op<?>>, Node.Op> nodes = new HashMap<>();
		...
		//private
		default void collectPathMapRec(Node.Op sgn, Set<Artifact.Op<?>> path, Map<Set<Artifact.Op<?>>, Node.Op> nodes) {
			if (sgn.getPol() == this.getPol()) // already visited
				return;
		...
		//private
		default void collectSymbolsRec(Node.Op sgn, Collection<Artifact.Op<?>> symbols) {
			if (sgn.getPol() == this.getPol()) // already visited
				return;
		...
		public default void trim(Collection<? extends Artifact.Op<?>> symbols) {
			Node.Op tempLeftRoot = this.createSequenceGraphNode(this.getPol());
			...
			Map<Set<Artifact.Op<?>>, Node.Op> rigthNodes = new HashMap<>();
		...
		//private
		default void trimRec(Collection<? extends Artifact.Op<?>> symbols, Set<Artifact.Op<?>> path, Map<Set<Artifact.Op<?>>, Node.Op> rightNodes, Node.Op left, Node.Op right) {
			...
			for (Transition.Op leftEntry : left.getChildren()) {
				if (symbols.contains(leftEntry.getKey())) { // put it into right
					...
					Node.Op rightChild = rightNodes.get(newPath);
					if (rightChild == null) {
		 ...
		 */
		public default void copy(SequenceGraphOld sg) {
			...
			Map<Set<Artifact.Op<?>>, Node.Op> leftNodes = new HashMap<>();
		...
		//private
		default void copyRec(Set<Artifact.Op<?>> path, Map<Set<Artifact.Op<?>>, Node.Op> leftNodes, Node.Op leftNode, Node.Op rightNode, boolean newPol) {
			if (rightNode.getPol() == newPol) // already visited
				return;

			rightNode.setPol(newPol); // mark as visited

			for (Transition.Op rightEntry : rightNode.getChildren()) {
				Set<Artifact.Op<?>> newPath = new HashSet<>(path);
				newPath.add(rightEntry.getKey());

				Node.Op leftChildNode = leftNodes.get(newPath);
	...
	public interface Node extends Persistable {
		...
		public Collection<? extends SequenceGraphOld.Transition> getChildren();

		public interface Op extends Node {
			@Override
			public Collection<SequenceGraphOld.Transition.Op> getChildren();

			public boolean getPol();

			public void setPol(boolean pol);

			public Transition.Op addTransition(Artifact.Op<?> key, SequenceGraphOld.Node.Op value);
		}
	}
	...
	public interface Transition extends Persistable {
		...
		public SequenceGraphOld.Node getValue();
		...
		public interface Op extends Transition {
			...
			@Override
			public SequenceGraphOld.Node.Op getValue();
		}
	}
